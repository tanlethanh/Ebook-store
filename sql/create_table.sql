...
alter table Books
    ...
    add constraint books_suppliers_fk
...
create table SpecificCategories
(
...
alter table Books
    ...
    add constraint specific_categories_fk
        foreign key (specific_category_id, category_id)
            references SpecificCategories (id, category_id)
...
(
    ...
    birthday     date,
    gender       enum ('M', 'F', 'A'), # male, female, another
    created_date datetime default now(),
    updated_date datetime default now()
);
...
(
    ...
    user_id  varchar(36),
    constraint address_users_fk foreign key (user_id) references Users (id)
        on delete cascade on update cascade
);
...
(
    ...
    updated_date        datetime                                               default now(),
    state               enum ('INIT', 'CONFIRM', 'SHIPPING', 'DONE', 'CANCEL') default 'INIT',
    ...
    user_id             varchar(36),
    constraint orders_address_fk foreign key (address_id) references Address (id)
...
(
    ...
    constraint orderbooks_orders_fk foreign key (order_id) references Orders (id)
        on delete cascade
        on update cascade,
    constraint orderbooks_books_fk foreign key (book_id) references Books (id)
...
(
    ...
    constraint comments_users_fk foreign key (userId) references Users (id)
        on delete cascade
        on update cascade,
    constraint comments_books_fk foreign key (bookId) references Books (id)
        on delete cascade
        on update cascade
);
