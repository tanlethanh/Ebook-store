...
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `check_order`(in cus_name varchar(255), cus_phone varchar(255))
BEGIN

    declare count_orders int;
    select count(orders.receiver_name) into count_orders from orders where orders.receiver_name = cus_name;

    -- Check exist order by name
    if (cus_name = '') then
    ...
    if (count_orders = 0) then
        signal sqlstate '45000'
            set message_text = 'Name is not exist';
    end if;

    -- Check phone number
    if (regexp_like(cus_phone, '^[[:digit:]]{10}$') = 0) then
        signal sqlstate '45000'
    ...
    from orders
    where orders.receiver_name = cus_name
      and orders.phone_number = cus_phone
